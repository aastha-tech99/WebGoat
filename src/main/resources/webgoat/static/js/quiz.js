            ...
            $.each(questionsObj, function(i, obj) {
                $.each(obj, function(j, quest) {
                  ...
                  $.each(quest.solutions, function(k, solution) {
                    var safeLabel = "Solution " + k + ": " + escapeHtml(solution);
                    var safeValue = escapeHtml("Solution " + k + ": " + solution);
                    html += '<input id="question_' + j + '_' + k + '_input" type="radio" name="question_' + j +'_solution" value="' + safeValue + '" required><label for="question_' + j + '_' + k + '_input">' + safeLabel + '</label><br>';
                  });
                  html += "</fieldset></div>";
                });
            });
